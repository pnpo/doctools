package pt.uminho.di.reolangeditor.editors.reconciler;
...
public class ReoLangReconcilingStrategy implements IReconcilingStrategy, IReconcilingStrategyExtension{

	private static final String PROBLEM="pt.uminho.di.reolangeditor.annotations.error"; //$NON-NLS-1$
	private static final String WARNING="pt.uminho.di.reolangeditor.annotations.warning"; //$NON-NLS-1$
	...
	public ReoLangReconcilingStrategy(ISourceViewer viewer, ReoLangEditor editor) {
		this.viewer = viewer;
	...
	public void reconcile(IRegion partition) {
		...
		try{
			...
			this.editor.setReoLangOutlineModel(rootElement);
			
			/*Fuzzy parser for CONSTRUCTING PATTERN GRAPH*/
			//Extract contextual text...
			
			
			int offset = getOffsetAtSWTThread();
			SimpleSymbol contextSymbol = ReoLangContextUtil.findMainContext(this.editor, offset);
			String text = ReoLangContextUtil.extractTextInContext(this.viewer, contextSymbol, offset);
			PatternViewer pv = new PatternViewer(file, text, rootElement);
			pv.createPatternViewer();
			this.editor.setSelectedPatternGraphModel(pv.getGraph());
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	private int getOffsetAtSWTThread(){
		...
		if(display.getThread() == Thread.currentThread()){
			...
			display.syncExec(tog) ;
		}
		
		while(offset == -1){ //will run only once because threads are executing synchronously 

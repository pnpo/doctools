 ...
 *
 */
public class CooPLaCompletionProcessor implements IContentAssistProcessor {

	
	private static String[] coopla_keywords = {"channel", 	"pattern", 	"import", 	"extends",
												"as", 		"NULL", 	"in", 		"use", 		
	...
	public CooPLaCompletionProcessor(CooPLaEditor e) {
		this.editor = e; 
	}
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		...
		ArrayList<ICompletionProposal> result = new ArrayList<ICompletionProposal>(coopla_keywords.length);
		
		SimpleSymbol context = CooPLaContextUtil.findMainContext(this.editor, offset);
	...
	private  ArrayList<ICompletionProposal> intellisenseByDEFAULT(int offset, String word, int word_length, SimpleSymbol context) {
		...
		ArrayList<SimpleSymbol> symbols = CooPLaContextUtil.extractSuitableSymbols(editor, context, word);
		ArrayList<String> ids = new ArrayList<String>();
		for(SimpleSymbol s : symbols){
			ids.add(s.getId());
		}
			
		String[] words = new String[ids.size()+coopla_keywords.length];
		ids.toArray(words);
		System.arraycopy(coopla_keywords, 0 , words, ids.size(), coopla_keywords.length);
		
		for (int i= 0; i < words.length; i++) {
			if(word_length > 0 && ((i >= ids.size() && coopla_keywords[i-ids.size()].startsWith(word)) || i < ids.size())){	
	...
	private ArrayList<ICompletionProposal> intellisenseForEQUAL(int offset, SimpleSymbol context, IDocument doc) {
		...
		String text_in_context = CooPLaContextUtil.extractText(context.getOffsetStop(), offset, doc);
		String text_after_context = CooPLaContextUtil.extractTextUntilContextIsFinished(offset, doc);
